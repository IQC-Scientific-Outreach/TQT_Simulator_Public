...
class TimeTagger:
    ...
    def generate_tag_array(self, time=1.0):
        ...
        Returns an (N, 2) int64 array of [channel, time_bin], sorted by time.
        This is the single source of truth for tag generation; both save_tags
        (desktop, writes to disk) and the web histogram build on it.
        """
        base_rate = 0.0
        if self.laser and getattr(self.laser, "is_emission_on", False):
    ...
    def save_tags(self, io=None, filename="tags", time=1.0, convert=True):
        """
        Generate physics-based tags for the current quantum state and write them.
        """
